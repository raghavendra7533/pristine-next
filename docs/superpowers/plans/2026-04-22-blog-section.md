...
**Goal:** Build a fully static `/blog` index and 8 `/blog/[slug]` article pages in the Next.js app, migrating content from PHP, rewriting for plain human language, and adding complete SEO/GEO metadata.

**Architecture:** All blog data (content + metadata) lives in `lib/blogs.ts` as a typed array. A dynamic route `app/blog/[slug]/page.tsx` uses `generateStaticParams` + `generateMetadata` for SSG. Two shared components handle rendering: `BlogCard` for listing views and `BlogContent` for article bodies.
...
|--------|------|---------------|
| Create | `lib/blogs.ts` | Single source of truth: types + all 8 posts |
| Create | `components/blog/BlogCard.tsx` | Card used on index and related-posts |
| Create | `components/blog/BlogContent.tsx` | Renders `Section[]` as semantic HTML |
| Create | `app/blog/page.tsx` | `/blog` index: metadata, JSON-LD ItemList, grid |
| Create | `app/blog/[slug]/page.tsx` | `/blog/[slug]`: generateStaticParams, generateMetadata, article layout |

---

## Task 1: Types and blog data (`lib/blogs.ts`)

**Files:**
- Create: `lib/blogs.ts`

- [ ] **Step 1: Create `lib/blogs.ts` with types and all 8 blog posts**
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          '78% of B2B buyers want salespeople to act as trusted advisors. They want vendors who understand their specific situation, industry context, and goals. A template with their name in the subject line doesn\'t do that.',
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          'Mid-market companies face a specific set of constraints that enterprise competitors don\'t.',
        ],
        list: [
          'Budget pressure — every dollar needs to show ROI immediately. The choice between hiring a rep, investing in tools, or running a campaign isn\'t strategic — it\'s a constraint.',
          'Credibility deficit — every sales call starts with an implicit "Who are you?" Even when your product is genuinely better, you\'re fighting the old saying that nobody ever got fired for buying the market leader.',
          'Lean teams — while enterprise companies have specialized roles for everything from lead generation to customer success, you\'re doing it all with a small team wearing multiple hats.',
        ],
      },
      {
        ...
        body: [
          ...
          'One VC-backed sales leader put it plainly: "Missing quota isn\'t just about bonuses here. It\'s about proving our entire business model works."',
          'A shocking percentage of mid-sized companies still don\'t have a properly configured CRM. Imagine running a high-velocity sales team without reliable pipeline data — managing deals with spreadsheets, sticky notes, and memory. Mike Rodriguez, a veteran sales rep turned consultant, said it perfectly: "Enterprise reps complain about their CRM. Mid-market reps dream of having a CRM to complain about."',
        ],
      },
      {
      ...
      {
        heading: 'You don\'t need enterprise resources to compete',
        body: [
    ...
    sections: [
      ...
      },
      {
        ...
        list: [
          ...
          'Build credibility with proof — case studies, customer stories, and specific ROI examples matter more for mid-market companies than for established brands. Buyers need social proof when they can\'t rely on name recognition.',
        ],
      },
      {
...
```bash
cd "Pristinedata landing/pristine-next" && npx tsc --noEmit 2>&1 | head -20
```

Expected: no errors referencing `lib/blogs.ts`.

- [ ] **Step 3: Commit**

```bash
git add "Pristinedata landing/pristine-next/lib/blogs.ts"
git commit -m "feat: add blog data model and rewritten content for all 8 posts"
```
...
```bash
git add "Pristinedata landing/pristine-next/components/blog/BlogCard.tsx"
git commit -m "feat: add BlogCard component"
```

---

## Task 3: `BlogContent` component

**Files:**
- Create: `components/blog/BlogContent.tsx`
...
- [ ] **Step 3: Check if `@tailwindcss/typography` is installed**

```bash
cd "Pristinedata landing/pristine-next" && cat package.json | grep typography
```
...
Then check if `tailwind.config.*` already includes it. If there is a `tailwind.config.ts` or `tailwind.config.js`, add `require('@tailwindcss/typography')` to the plugins array. If the project uses CSS-based Tailwind v4 config (check `postcss.config.mjs` and `globals.css`), add `@plugin "@tailwindcss/typography";` to `globals.css`.

To detect which version:
```bash
cd "Pristinedata landing/pristine-next" && cat node_modules/next/package.json | grep '"tailwindcss"'
```

- [ ] **Step 4: Commit**

```bash
git add "Pristinedata landing/pristine-next/components/blog/BlogContent.tsx"
...
```bash
git add "Pristinedata landing/pristine-next/app/blog/page.tsx"
git commit -m "feat: add /blog index page with metadata and JSON-LD"
```

---

## Task 5: Individual article page (`/blog/[slug]`)
...
export default async function BlogArticlePage({ params }: Props) {
  ...
  // Find the last section (CTA) to render separately
...
```bash
cd "Pristinedata landing/pristine-next" && npm run dev
```

Check in browser:
- `http://localhost:3000/blog` — shows 8 cards with titles and images
- `http://localhost:3000/blog/quota-killers-what-top-b2b-reps-do-differently` — shows full article, CTA, related posts
- `http://localhost:3000/blog/nonexistent-slug` — shows Next.js 404

- [ ] **Step 4: Run build to confirm static export works**

```bash
cd "Pristinedata landing/pristine-next" && npm run build 2>&1 | tail -30
```

Expected: build completes with `●` static markers next to `/blog` and all `/blog/[slug]` routes. No errors.

- [ ] **Step 5: Commit**

```bash
git add "Pristinedata landing/pristine-next/app/blog"
git commit -m "feat: add /blog/[slug] article pages with SSG, metadata, and JSON-LD"
```
...
|---|---|
| `/blog` index page | Task 4 |
| `/blog/[slug]` individual pages | Task 5 |
| `generateStaticParams` for SSG | Task 5 |
| Per-page `generateMetadata` | Task 5 |
| JSON-LD `Article` per post | Task 5 |
| JSON-LD `ItemList` on index | Task 4 |
| OG + Twitter cards | Tasks 4 & 5 |
| Human prose, no keyword stuffing | Task 1 (content rewrite) |
| Strip per-section product plugs | Task 1 (content rewrite) |
| One CTA per article | Task 1 (last section) + Task 5 (rendered separately) |
| GTM only, no HR/job search | Task 1 (content reviewed) |
| Easy to add future blogs | Task 1 (`getBlogBySlug`, `blogs` array — prepend object) |
| Footer link to `/blog` | Out of scope (noted in spec) |
| `BlogCard` reused for index and related posts | Tasks 2, 4, 5 |
| Semantic HTML h1→h2→h3 | Tasks 3, 5 |
| Related posts (3 cards) at bottom of article | Task 5 |
| Better slugs | Task 1 |

All spec requirements covered. No placeholders. Types consistent across all tasks (`BlogPost`, `Section`, `getBlogBySlug`, `getRelatedPosts`).
