...
const approachRows: [string, string, string][] = [
  ['Data quality', 'Parallel live enrichment: best result per field across 3 providers', 'Single-source database snapshot; records age between refreshes'],
  ['Pricing model', 'Flat monthly: all enrichment and signals included', 'Free tier + paid plans; credits for exports and enrichment'],
  ['GTM flexibility', 'Prospecting, signals, AI personalization, and campaigns in one workflow', 'Large database + sequencer; no pre-call intelligence layer'],
  ['AI personalization', 'Opportunity Playbook: structured pre-call brief before every call', 'Basic AI email suggestions'],
...
const featureRows: [string, string, string][] = [
  ...
  ['Pricing model', 'Flat monthly', 'Free tier + paid plans; credits for exports'],
...
const faqs = [
  ...
  },
  {
    ...
    a: "Apollo's paid plans are credit-based: you pay per export, per enrichment action, and for higher-tier features. Costs compound as your team scales. Pristine's pricing is flat: enrichment, signals, and campaigns are all included. No per-credit billing for standard usage.",
  },
  {
...
const apolloPageJsonLd = [
  {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Pristine vs Apollo', item: 'https://pristinedata.ai/compare/pristine-vs-apollo' },
    ],
  },
  {
    '@context': 'https://schema.org',
...
export default function PristineVsApolloPage() {
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(apolloPageJsonLd) }} />
      <Navbar />
      <main className="pt-16">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <section className="py-16 text-center">
            ...
            <p className="text-xs font-semibold uppercase tracking-widest text-indigo-500 mb-4">Apollo Alternative</p>

            <h1 className="text-4xl sm:text-5xl font-bold text-slate-900 dark:text-white leading-tight mb-4 max-w-2xl mx-auto">
              Pristine Data vs Apollo
            </h1>
            <p className="text-xl text-slate-500 dark:text-slate-400 mb-3">Apollo is where outbound starts.<br className="hidden sm:block" /> Pristine is where it starts working.</p>
          ...
          <section className="py-16 border-t border-slate-100 dark:border-slate-800">
            ...
            <p className="text-base text-slate-500 dark:text-slate-400 mb-8">If you&apos;re evaluating Apollo or searching for Apollo alternatives in 2025–2026, here&apos;s the honest breakdown.</p>
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-6">
                <div className="flex items-center gap-2 mb-4">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src="https://img.logo.dev/apollo.io?token=pk_R0FhQgSqRMmR86Lw1NOJNg" alt="Apollo" className="w-5 h-5 object-contain" />
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">Choose Apollo if…</p>
                </div>
                <ul className="space-y-2.5">
                  {[
                    'You are just starting outbound and need a low-barrier, affordable way to build lists and run sequences.',
                    'Your team is small and the free tier covers the volume you need right now.',
                    'You primarily need a database and sequencer and do not yet need pre-call intelligence or live enrichment.',
                  ].map((item) => (
              ...
              <div className="rounded-2xl border border-indigo-200 dark:border-indigo-900/50 bg-indigo-50/40 dark:bg-indigo-950/20 p-6">
                ...
                <ul className="space-y-2.5">
                  {[
                    'You have outgrown the free tier and export credits are compounding faster than your pipeline.',
                    'You need live enrichment, not a snapshot: stale data is costing you bounce rates.',
                    'Your reps need structured pre-call intelligence, not just a contact record and a sequence.',
                  ].map((item) => (
          ...
          <section id="comparison" className="py-16 border-t border-slate-100 dark:border-slate-800">
            ...
            <div className="overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-800">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {approachRows.map(([label, pristine, apollo], i) => (
          ...
          <section className="py-16 border-t border-slate-100 dark:border-slate-800">
            ...
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Pristine Data: An Apollo Alternative Built for the Next Stage</h2>
            <div className="space-y-4 text-base text-slate-600 dark:text-slate-400 leading-relaxed">
              <p>If you&apos;re evaluating Apollo alternatives, it&apos;s usually because credits are compounding, data is going stale, or your team needs more than a contact record. Apollo built its reputation on a large database and a generous free tier: and we use Apollo as one of our enrichment providers for exactly this reason: the coverage is real.</p>
              <p>The limitations show when you need more than a contact record. Apollo&apos;s data is a snapshot: records age between refreshes, and you discover the bounce rate on delivery. There is no pre-call intelligence, no structured buying signal layer integrated into prospecting, and no AI personalization at campaign level. Export credits compound as your team scales.</p>
              <p className="font-medium text-slate-800 dark:text-slate-200">Pristine is what comes next. If you&apos;re searching for an Apollo alternative that gives you live multi-source enrichment, buying signals where the list is built, and a pre-call brief before every call: that is what Pristine is built for.</p>
            </div>
          </section>
